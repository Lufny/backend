...
Post.prototype.write = function(callback) {
  ...
  if (!util.checkValues(this.post, ["text", "author", "ipAddr"]))
    return callback({error: "Some value is missing"});
  ...
  postmod.save(function(err, docs) {
    ...
    for (n=0; n<tags.length; n++) {
      cnot = new Notification({not: {type: "tag", id: docs._id, uid: author, read: false, date: date.getTime()}, where: {username: tags[n]}});
      cnot.add(blank);
    }
